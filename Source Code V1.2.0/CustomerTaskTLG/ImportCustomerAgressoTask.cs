...
namespace CustomerTaskTLG
{
    public class ImportCustomerAgressoTask : Worker<Parameters, Settings>
    {
                ...
                using (var connection = (DbConnection)Settings.DbConnection.CreateConnection())
                {
                    ...
                    using (var reader = await command.ExecuteReaderAsync())
                    {
                        while (reader.Read())
                        {
                            ...
                            //Condition created to avoid processing a customer if client hasn't been set
        ...
        {
            var command = connection.CreateCommand();
            command.CommandText = $@"SELECT right(rtrim(isnull(c.customerid, '')),7) apar_id,
            isnull(c.customerid,'') as full_apar_id,
            case when c.name<> '' then left(rtrim(isnull(c.name,'')),255) else right(rtrim(isnull(c.customerid,'')),7) end apar_name,
            'R' apar_type,
            left(isnull(m.IncludedBranches, ''),25) client,
            left(rtrim(case when c.NetSuiteExternalReference <> '' then isnull(c.NetSuiteExternalReference,'') else '.' end),100) ext_apar_ref,
            'CH' pay_method,
            right(rtrim(isnull(c.customerid,'')),7) short_name,
            ...
            left(rtrim(isnull(c.email,'')),255) e_mail,
            left(rtrim(isnull(c.webaddress,'')),255) url_path,
            left(rtrim(isnull(a.phone,'') ), 35) telephone_1,
            left(rtrim(isnull(a.attention,'')),255) description,
            isnull(c.agentid,'') agent,
            0 sequence_no,
            isnull(c.applicationclient,'') applicationclient,
            'AGENT' rel_name
            FROM STG_Customer c LEFT OUTER JOIN STG_CustomerAddress a on c.customerid = a.customerid and c.applicationclient = a.applicationclient
            LEFT OUTER JOIN STG_MidOfficeControl m on c.applicationclient = m.applicationclient and m.application = 'AGRESSO'
