...
namespace VendorTaskTLG
{
    public class ImportVendorAgressoTask : Worker<Parameters, Settings>
    {
                        ...
                        while (reader.Read())
                        {
                            ...
                            if (response.StatusCode == System.Net.HttpStatusCode.Created)
                            {
                                ...
                                using (var connection_update = (DbConnection)Settings.UBWConnection.CreateConnection())
                                {
                                    ...
                                    Logger.Information("- Inserting operation code and message into STG_VendorMaster: " + codeResponse + " - " + codeResponse);
                                    //Insert response into STG_VendorMaster
                                    ResponseExec responseExec = new ResponseExec();
                                    responseExec.code = codeResponse;
                                    responseExec.message = codeResponse;
                                    DbCommand command2 = SetVendorResponseCommand(connection_update, responseExec, fullCustomerId, applicationclient);
                            ...
                            else
                            {
                                ...
                                if (response.Content.Contains("\"code\":3010") && response.StatusCode == System.Net.HttpStatusCode.BadRequest)
                                {
                                    ...
                                    IRestResponse responseGet = ExecuteGetRestApi(vendor);
                                    ...
                                    {
                                        await connection_update.OpenAsync();
                                        Logger.Information("- Inserting operation code and message into STG_VendorMaster: " + codeResponse + " - " + System1ErrorMessage);
                                        ResponseExec responseExec = new ResponseExec();
                                        responseExec.code = codeResponse;
        ...
        public DbCommand GetVendorDataCommand(DbConnection connection)
        {
            ...
            'P' apar_type,
            v.applicationclient,
            left(rtrim(M.IncludedBranches),25) client,
            left(rtrim(case when v.Vendorid <> '' then v.vendorid else '.' end), 100) ext_apar_ref,
            'CH' pay_method,
            left(rtrim(v.vendorid),3) short_name,
            ...
            'JZ' rel_attr_id
            FROM STG_VendorMaster v left JOIN STG_VendorMasterAddress a on v.vendorid = a.vendorid and v.applicationclient = a.applicationclient
            join STG_MidOfficeControl m on v.applicationclient = m.applicationclient and m.application = 'AGRESSO'
            left join STG_RevGrpReference rg on v.suppliertype = rg.NSSupplierType and
        ...
        public DbCommand SetVendorResponseCommand(DbConnection connection, ResponseExec responseExec, String fullCustomerId, String applicationclient)
        {
            ...
            {
                update = $"UPDATE STG_VendorMaster SET System1RefernceID = '{responseExec.code}', System1ErrorMessage = NULL WHERE NetSuiteExternalReference = '{fullCustomerId}' AND ApplicationClient = '{applicationclient}'";
            }
            //if code response is 3010, we set the message of update and success
            else if (responseExec.code == "3010")
            {
                update = $"UPDATE STG_VendorMaster SET System1RefernceID = 'SUCCESS', System1ErrorMessage = 'Vendor updated' WHERE NetSuiteExternalReference = '{fullCustomerId}' AND ApplicationClient = '{applicationclient}'";
            }
        ...
        public IRestResponse ExecutePostRestApi(Vendor vendor)
        {
            ...
            string requestUrl = $"http://{Settings.APIAddress}/v1/suppliers/";
        ...
        public IRestResponse ExecuteGetRestApi(Vendor vendor)
        {
            //Setting API URL, Authentication and Serialize customer object to JSON
            string requestUrl = $"http://{Settings.APIAddress}/v1/suppliers/{vendor.SupplierId}";
        ...
        public IRestResponse ExecutePatchRestApi(Vendor vendor, string jsonBody)
        {
            //Setting API URL, Authentication and Serialize customer object to JSON
            string requestUrl = $"http://{Settings.APIAddress}/v1/suppliers/{vendor.SupplierId}?_action=patch";
        ...
        public IRestResponse CompareVendorDifferencesAndUpdate(Vendor newVendor, Vendor oldVendor)
        {
            ...
            foreach (var diff in result.Differences)
            {
                ...
                {
                    if (propertyName != "CustomFieldGroups" && propertyName != "LastUpdated" && propertyName != "ContactPoints[0].LastUpdated"
            ...
            int newindex = newrelatedValues.FindIndex(a => a.RelationId == "JZ");
            int oldindex = oldrelatedValues.FindIndex(a => a.RelationId == "JZ");
            if (newindex >= 0)
            {
            ...
            if (fulljsonBody != "[]")
            {
                ...
                resUpdate = ExecutePatchRestApi(newVendor, fulljsonBody);
